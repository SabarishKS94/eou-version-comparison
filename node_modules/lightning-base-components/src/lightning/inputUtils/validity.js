...
function getConstraintsSortedByPriority() {
    return [
        'customError',
        'badInput',
        'badNumericInput',
...
function resolveBestMatch(validity) {
    ...
    if (validity && validity.valid === false) {
        ...
        getConstraintsSortedByPriority().some((stateName) => {
...
export class FieldConstraintApi {
    _inputComponentProvider;
    _constraintsProvider;
    _constraint;
    _customValidityMessage;
    _inputComponentElement;
    constructor(inputComponentProvider, constraintProviders) {
        ...
        if (!this._constraintsProvider.customError) {
            this._constraintsProvider.customError = () => typeof this._customValidityMessage === 'string' &&
...
export class FieldConstraintApiWithProxyInput {
    ...
    _attributes;
    _attributeUpdater;
    _privateConstraintApi;
    constructor(inputComponent, overrides = {}, inputElementName = 'input') {
        if (inputElementName !== 'textarea' && inputElementName !== 'input') {
            throw new Error('FieldConstraintApiWithProxyInput is not supported for this element');
        }
    ...
    _setAttribute(attributeName, value) {
        if (value !== null && value !== undefined && value !== false) {
            if (attributeName === 'value') {
                ...
                reflectAttribute(this._proxyInput, attributeName, value.toString());
            }
            else {
                reflectAttribute(this._proxyInput, attributeName, value);
            }
        }
        else {
            reflectAttribute(this._proxyInput, attributeName);
        }
    }
    get _constraintApi() {
        if (!this._privateConstraintApi) {
            ...
            const computeConstraintWithProxyInput = (constraintName) => {
                ...
                const isDisabledOrReadOnly = this._proxyInput.hasAttribute('disabled') ||
            ...
            const constraintsProvider = getConstraintsSortedByPriority().reduce((provider, constraint) => {
