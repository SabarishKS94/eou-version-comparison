
:host([data-render-mode="shadow"]) .slds-nav-vertical__action {
  --slds-c-icon-color-foreground-default: currentcolor;

  position: relative;
  display: flex;
  align-items: center;
  width: 100%;
  padding-block-start: var(--slds-c-verticalnavigationitem-spacing-blockstart, var(--slds-g-spacing-2, 0.5rem));
  padding-block-end: var(--slds-c-verticalnavigationitem-spacing-blockend, var(--slds-g-spacing-2, 0.5rem));
  padding-inline-start: var(--slds-c-verticalnavigationitem-spacing-inlinestart, var(--slds-g-spacing-6, 2rem));
  padding-inline-end: var(--slds-c-verticalnavigationitem-spacing-inlineend, var(--slds-g-spacing-5, 1.5rem));
  border-block-start: var(--slds-g-sizing-border-1, 1px) solid transparent;
  border-block-end: var(--slds-g-sizing-border-1, 1px) solid transparent;
  border-radius: 0;
  box-shadow: inset 0 0 0 var(--slds-g-color-brand-base-60, #1b96ff);
  cursor: pointer;
  color: var(--slds-g-color-neutral-base-40, #5c5c5c);
}

:host([data-render-mode="shadow"]) .slds-nav-vertical__action_overflow {
  --slds-c-button-neutral-font-lineheight: default;
  --slds-c-icon-color-foreground: var(--slds-g-color-neutral-base-50, #747474);

  width: inherit;
  border-block-start: 1px solid transparent;
  border-block-end: 1px solid transparent;
}

:host([data-render-mode="shadow"]) .slds-nav-vertical__action-text {
  color: var(--slds-g-color-brand-base-50, #0176d3);
}

:host([data-render-mode="shadow"]) .slds-nav-vertical__action:hover {
  box-shadow: inset 2px 0 0 var(--slds-g-color-brand-base-60, #1b96ff);
  color: currentcolor;
}

:host([data-render-mode="shadow"]) .slds-nav-vertical__action:focus {
  text-decoration: underline;
  color: currentcolor;
}

:host([data-render-mode="shadow"].slds-nav-vertical__item) {
  position: relative;
}

:host([data-render-mode="shadow"].slds-nav-vertical__item:focus-within) {
  box-shadow: inset 0 0 0 1px var(--slds-g-color-brand-base-60, #1b96ff);
}

:host([data-render-mode="shadow"].slds-nav-vertical__item:hover),:host([data-render-mode="shadow"].slds-nav-vertical__item.slds-is-active) {
  background: var(--slds-c-verticalnavigationitem-color, var(--slds-g-color-brand-base-95, #eef4ff));
}

:host([data-render-mode="shadow"].slds-is-active) .slds-nav-vertical__action:not(:focus) {
  border-color: var(--slds-c-verticalnavigationitem-border-color, transparent);
}

:host([data-render-mode="shadow"].slds-nav-vertical__item.slds-is-active) .slds-nav-vertical__action {
  font-weight: bold;
  box-shadow: inset 4px 0 0 var(--slds-g-color-brand-base-60, #1b96ff);
}
