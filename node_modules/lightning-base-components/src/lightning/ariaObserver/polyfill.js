...
function getParentIgnoringShadowRoot(element) {
    if (typeof element.assignedNodes !== 'function' &&
...
function mirrorNode(node, existingNode) {
    ...
    const newAttributes = Object.fromEntries([...Object.entries(getAttributes(node))].filter(([name]) => name.toLowerCase().startsWith('aria-') ||
...
function mirrorNodes(mirroredEnvironment, task) {
    const { redundantChildNodes, nodesToMirroredNodes, mirrorRoot } = mirroredEnvironment;
    task.redundantChildNodes.forEach((node) => redundantChildNodes.add(node));
    const rootToNodes = new Set(task.relationships
        .map(({ toNodes }) => toNodes)
        .flat()
        .filter(Boolean)
        .filter((_) => !redundantChildNodes.has(_)));
    rootToNodes.forEach((toNode) => {
        const trackedNodes = new Set([...redundantChildNodes, toNode]);
        const { node: existingMirroredNode = null, usage = 0 } = nodesToMirroredNodes.get(toNode) || {};
        const { mirroredNode, trackedNodesToMirroredNodes } = patchMirrorDomTree(toNode, existingMirroredNode, trackedNodes);
...
function linkAndObserveNodes(mirroredEnvironment, task) {
    ...
    task.relationships.forEach((collatedRelationship) => {
        const { fromNode, toNodes, relationship, track, signal, mirrorOnly } = collatedRelationship;
        if (toNodes?.length) {
            if (!mirrorOnly) {
                const mirroredNodes = toNodes.map((toNode) => nodesToMirroredNodes.get(toNode).node);
                const { linkedNodeIds, unlinkedNodeIds } = updateAttribute(fromNode, mirroredNodes, relationship);
            ...
            const { unlinkedNodeIds } = updateAttribute(fromNode, [], relationship);
...
function updateNodeUsage(mirroredEnvironment, unlinkedNodes, linkedNodes = []) {
    ...
    nodesToMirroredNodes.forEach((mirroredNode, node) => {
        ...
        else if (unlinkedNodes.includes(mirroredNode.node.id) &&
