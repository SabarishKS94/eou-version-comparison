<template>
    <fieldset class={computedFieldsetClass} part="input-address">
                ...
                <template if:true={showDedicatedLookupAddressField}>
                    <div class="slds-form-element__row slds-grow">
                        <lightning-lookup-address
                            ...
                            disabled={disabled}
                            country-lookup-filter={countryLookupFilter}
                        ></lightning-lookup-address>
                    </div>
                    <span class="slds-assistive-text" role="status">
                        {_messageOnAddressUpdate}
                    </span>
                </template>
                <template
                    for:each={domFieldsMeta}
                    for:item="row"
                    for:index="index"
                >
                        ...
                        <template for:each={row} for:item="field">
                            <template if:true={field.isLookupAddress}>
                                <lightning-lookup-address
                                    ...
                                    onblur={handleBlur}
                                    onchange={handleAddress}
                                    onprivateuserinput={handleChange}
                                    country-lookup-filter={countryLookupFilter}
                                ></lightning-lookup-address>
                            </template>
