...
:host([data-render-mode="shadow"]) .slds-listbox_vertical .slds-listbox__option_entity {
  ...
  padding-inline: var(--slds-g-spacing-3, 0.75rem);
}
...
.slds-listbox__option[aria-disabled='true']:focus {
  color: var(--slds-c-listbox-option-color-disabled, var(--slds-g-color-disabled-2, #aeaeae));
...
:host([data-render-mode="shadow"]) .slds-listbox__icon-selected {
  opacity: 0;
  fill: var(--slds-g-color-accent-2, var(--lwc-brandPrimaryActive, var(--slds-g-color-brand-base-50)));
}
...
:host([data-render-mode="shadow"]) .slds-listbox__option-meta {
  display: block;
  margin-block-start: calc(var(--slds-g-spacing-1, 0.25rem) * -1);
...
:host([data-render-mode="shadow"]) .slds-listbox_selection-group {
  position: relative;
  padding: 0;
  padding-inline-end: calc(var(--slds-g-spacing-8, 3rem) + var(--slds-g-spacing-3, 0.75rem));
  height: 1.875rem;
  overflow: hidden;
}
